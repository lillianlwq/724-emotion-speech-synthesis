...
# Alternative way of using Transforemer, using the torch.nn.Transformer to implement the
# Define a custom model using Transformer
class MyTransformerModel(nn.Module):
    def __init__(self, input_dim, output_dim, d_model, nhead, num_encoder_layers, dim_feedforward):
        super(MyTransformerModel, self).__init__()
        self.transformer = nn.Transformer(d_model=d_model, nhead=nhead, num_encoder_layers=num_encoder_layers,
                                          dim_feedforward=dim_feedforward)
        self.fc_in_src = nn.Linear(input_dim, d_model)
        self.fc_in_tgt = nn.Linear(output_dim, d_model)
